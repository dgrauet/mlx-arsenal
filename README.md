...
| `mlx_arsenal.diffusion` | `get_timestep_embedding`, `TimestepEmbedding`, `get_sampling_sigmas`, `dynamic_shift_schedule`, `FlowMatchEulerDiscreteScheduler`, `DDIMScheduler`, `euler_step`, `classifier_free_guidance`, `TeaCacheController`, `PerLayerAttentionCache`, `PerHeadAttentionCache`, `splice_heads` | Flow-matching + DDIM diffusion primitives, TeaCache, AST-style attention output caches |
